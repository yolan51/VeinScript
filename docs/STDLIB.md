...
| **Web** | `Http` (`@Request{path,method,body}` `@Html` `@Style` `@Script` `@Render` `@Response`) · `Elements` (markup builders `Heading` `Subheading` `Subsubheading` `Paragraph` `Button` `Link` `Image` `ListItem` `CodeBlock` `JumpLine` `Form` `Open` `OpenAs` `Close` `Raw`) · `Styles` (`Rule` `Css` → `@Style`) · `Scripts` (`Define` `Js` → `@Script`) — a builder's **output field** picks its channel (`markup`/`css`/`code`), so a stylesheet and a script assemble exactly as a page does. A builder cannot wrap other fragments, so a container is spelled as its two halves — `&Open`/`&OpenAs` … `&Close` — and `&Raw` stays the escape hatch for anything the vocabulary does not model | — |
| **WebTheme** | `Palettes` (`Slate` `Ink` — `:root` + `.dark` custom properties, nothing else) · `Foundation` (`Reset` `Motion`) · `Components` (`NavBar` `HeroArea` `Cards` `Controls` `Prose` `Layout` `Article` `Code` `Callout`) · `Behaviours` (`ThemeToggle` `Clock{id}` → `@Script`) — the look, kept out of `Vein.Web` so taking the channels never means taking the taste. Nothing says a `css =` field holds ONE rule, so a whole stylesheet is a single fragment and choosing a look is `bring`ing the pieces you want. Components never name a colour (they read `var(--brand)` etc.), which is what lets one palette swap restyle a site; `&ThemeToggle` is the other half of the palettes' `.dark` contract | — |
