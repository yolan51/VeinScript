...
- **Publicators = the shared API** (`shared` shapes/events/builders), reachable across bundles as
  ...
  **24 shapes · 27 events · 6 builders**, in **20 publicators**.
...
| **Console** | `Io` (`@Print{text}` `@Input{text}`) — console I/O; the runtime bridges these to stdout/stdin under `veinc run` | — |
