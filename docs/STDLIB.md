...
Vein.WebTheme — the LOOK: palettes, foundation, components (a separate `use`, so channels ≠ taste)
...
| **Web** | `Http` (`@Request{path,method,body}` `@Html` `@Style` `@Script` `@Render` `@Response`) · `Elements` — every element is a **shape + a builder that includes it**: `$Heading` `$Subheading` `$Subsubheading` `$Paragraph` `$Link` `$ListItem` `$CodeBlock` `$Container` `$Markup` `$Form` with builders `Heading` `Subheading` `Subsubheading` `Paragraph` `Link` `ListItem` `CodeBlock` `Open` `OpenAs` `Close` `Raw` `Form`; `&Button`/`&Image` reuse `*Vein.UI.Widgets.$Button`/`$Image` rather than declaring a second one, and `&JumpLine` has no shape because it carries no data · `Styles` (`Rule` `Css` → `@Style`) · `Scripts` (`Define` `Js` → `@Script`) · `Js` — behaviour COMPOSED rather than quoted: `seq` `raw` `text` `expr` `addNumber` `addClass` `removeClass` `toggleClass` `goto` `alert` `log`, each a `fn` returning a JS statement, so `addNumber(domId(out), 1)` replaces a hand-spelled `"var e=document.getElementById('"+…"');e.innerText=(+e.innerText)+1"` and the element id still comes from the identity. Each takes an element ID rather than a JS expression, so the lookup binds to a local once instead of nesting into every position. A literal value is single-quoted and NOT escaped, so a value containing `'` breaks the output — `raw` is the way out until the language has a string-escape function — a builder's **output field** picks its channel (`markup`/`css`/`code`), so a stylesheet and a script assemble exactly as a page does. The include is a compile-time field expansion, so `bring Heading("…")` costs nothing extra; what it buys is that the *same* declaration can be `attach`ed, turning a page into a `target` query rather than a list of literals. A builder cannot wrap other fragments, so a container is spelled as its two halves — `&Open`/`&OpenAs` … `&Close` — and `&Raw` stays the escape hatch for anything the vocabulary does not model | — |
| **WebTheme** | `Palettes` (`Slate` `Ink` — `:root` + `.dark` custom properties, nothing else) · `Foundation` (`Reset` `Motion`) · `Components` (`NavBar` `HeroArea` `Cards` `Controls` `Prose` `Layout` `Article` `Code` `Callout`) · `Behaviours` (`ThemeToggle`, and `$Clock`+`Clock` → `@Script`) — the look, kept out of `Vein.Web` so taking the channels never means taking the taste. Nothing says a `css =` field holds ONE rule, so a whole stylesheet is a single fragment and choosing a look is `bring`ing the pieces you want. Components never name a colour (they read `var(--brand)` etc.), which is what lets one palette swap restyle a site; `&ThemeToggle` is the other half of the palettes' `.dark` contract | — |
